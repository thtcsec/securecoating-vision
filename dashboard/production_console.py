...
<div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #222C3E; padding-bottom:12px; margin-bottom:15px;">
    <div>
        ...
        <span style="color:#8C9BAE; font-size:12px;">Authoritative snapshot {snapshot['snapshot_id']} at {snapshot['generated_at_utc']}</span>
    </div>
    <div>
        <span class="stage-badge stage-done">AUTHORITATIVE API</span>
        <span class="stage-badge" style="background:{disposition_color}; color:#FFFFFF;">{disposition}</span>
    </div>
</div>
""",
        unsafe_allow_html=True,
    ...
    with m5:
        _metric_card("SPC Quality State", spc_status, "#FF1744", spc_class)
    with m6:
        _metric_card("Fail-Safe Health", sys_state, "#00E5FF", sys_class)
    ...
    if st.sidebar.button("Reload snapshot", use_container_width=True):
        st.rerun()
    ...
    with operate:
        ...
        if reasons:
            st.warning("Readiness blockers: " + "; ".join(reasons))
        else:
...
<div class="scada-panel">
    ...
    <p><b>Connection status:</b> {connection.get('status', 'UNKNOWN')}</p>
    <p><b>Endpoint:</b> <code>{connection.get('plc_ip', 'UNKNOWN')}:{connection.get('modbus_port', 'UNKNOWN')}</code></p>
    <p><b>Interlock latched:</b> {industrial.get('interlock_latched')}</p>
    <p><b>Interlock reason:</b> {industrial.get('interlock_reason') or 'none reported'}</p>
</div>
""",
                unsafe_allow_html=True,
            )
        ...
        if not actions:
            ...
            if st.button("Submit control command", type="primary"):
                try:
                    ...
                    st.error(f"Control request failed before an audit result was returned: {exc}")
                else:
...
<div class="scada-panel" style="border-left:4px solid #00E676;">
    <p><b>Certificate ID:</b> <code>{certificate.get('certificate_id')}</code></p>
    <p><b>Payload digest:</b> <code>{certificate.get('payload_hash_sha256')}</code></p>
    <p><b>HMAC:</b> <code>{certificate.get('hmac_digital_signature')}</code></p>
    <p><b>Quality grade:</b> {certificate.get('overall_quality_grade')} · <b>Pass rate field:</b> {pass_rate_field}</p>
    <p><b>Metric provenance:</b> {certificate.get('metric_provenance')}</p>
</div>
""",
                unsafe_allow_html=True,
            )
