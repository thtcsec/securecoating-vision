...
- Configured prototype-policy summary (not standards certification)
...
class DigitalRollCertificate:
    """Provisional tamper-evident roll quality manifest."""
    certificate_id: str
    roll_id: str
    batch_id: str
    electrode_type: str
    ...
    def generate_cryptographic_signature(self, secret_key: bytes = DEFAULT_FACTORY_SECRET_KEY) -> str:
        ...
        raw_bytes = json.dumps(
            self._signature_payload(), sort_keys=True, separators=(",", ":"), allow_nan=False
        ).encode("utf-8")
...
*Provisional manifest generated by SecureCoating-Vision. Tamper-evident within the trust boundary of the configured factory HMAC secret; not a regulatory certificate.*
"""
        return md
...
class RollCertificateGenerator:
    ...
    def build_certificate(
        ...
        root_cause_summary: str = "UNKNOWN",
        ...
        cert_id = (
            f"CERT_{roll_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}_"
