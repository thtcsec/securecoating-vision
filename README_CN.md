...
<!-- TEST_MANIFEST:START -->
当前软件验证快照为 285 项测试通过、0 项跳过、0 项失败（提交 `d463c8a8c94f`，工作树干净，源码差异 `none`，Python 3.11.9，耗时 83.01 秒）。权威记录为 [reports/test_manifest.json](reports/test_manifest.json)。该记录仅证明当前软件测试结果，不代表工厂性能、真实 PLC 行为、安全等级急停认证或生产资质。
<!-- TEST_MANIFEST:END -->
