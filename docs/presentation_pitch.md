...
*   **Answer:** "There were access and environment constraints for some of the provided platforms, so rather than make the submission depend on an environment I could not reproduce reliably, I kept the final evidence path local and deterministic. Cloud/HPC stay optional future acceleration. Do not name specific platforms as failing unless a judge asks."
