...
<!-- TEST_MANIFEST:START -->
The current software validation snapshot is 286 passing tests with 0 skips and 0 failures (commit `293ce7be0d96`, working tree clean, source diff `none`, Python 3.11.9, 105.42s). The authoritative record is [reports/test_manifest.json](reports/test_manifest.json). This does not constitute evidence of factory performance, physical PLC behavior, safety-rated E-stop operation, or production qualification.
<!-- TEST_MANIFEST:END -->
...
Competition organizers provided cloud and HPC support channels. The authoritative evidence path of this submission does not depend on those environments. The final prototype and validation workflow are reproducible on local commodity hardware, with GPU-capable workloads executed on an NVIDIA GeForce RTX 4050 Laptop GPU and CPU inference measured separately. Cloud/HPC remains an optional acceleration path for larger future experiments.

Immutable local environment provenance: [reports/local_compute_environment.json](reports/local_compute_environment.json) (generated by `scripts/record_local_compute_environment.py`; not a factory or TensorRT benchmark).
...
The staged implementation checklist, evidence matrix, release gates, and next-step plan are maintained in [docs/implementation_status.md](docs/implementation_status.md). The longer industrialization backlog lives in [docs/industrialization_path.md](docs/industrialization_path.md).

**Next validation gates** (not claimed complete):

- **Factory evidence** — independent roll-disjoint data + plant calibration.
- **Hardware validation** — PLC/HIL + target-edge performance measurement.
- **Research extensions** — risk–coverage calibration and downstream property prediction with real labels.
